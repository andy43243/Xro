    ...
    local function HandleInputBegan(Input)
        if Input.UserInputType == Enum.UserInputType.MouseButton1 or 
           Input.UserInputType == Enum.UserInputType.Touch then
            ...
            if Input.UserInputType == Enum.UserInputType.Touch then
                Dragger.BackgroundTransparency = 0.3
            end
        end
    end
    ...
    local function HandleInputChanged(Input)
        if IsDragging and (Input.UserInputType == Enum.UserInputType.MouseMovement or 
           (Input.UserInputType == Enum.UserInputType.Touch and TouchStartId and Input.UserInputState == Enum.UserInputState.Change)) then
            ...
            -- Smooth dragging with sensitivity
            local Sensitivity = Input.UserInputType == Enum.UserInputType.Touch and 1.0 or 1.0
...
local function MakeResizeable(Dragger, Object, MinSize, OnTick, OnStop)
    ...
    -- Make resize handle more visible and larger for mobile
...
function Assets:Window(ScreenAsset,Window)
	...
	-- Enhanced mobile dragging - make entire window draggable on mobile
	if UserInputService.TouchEnabled then
		MakeDraggable(WindowAsset.Background,WindowAsset,function(Position)
			Window.Position = Position
		end)
		
		-- Also make the main window frame draggable
		MakeDraggable(WindowAsset,WindowAsset,function(Position)
			Window.Position = Position
		end)
	end
	...
	-- Create Toggle Button (appears outside UI)
	local ToggleButton = Instance.new("ImageButton")
	ToggleButton.Name = "UIToggleButton"
	ToggleButton.Size = UDim2.fromOffset(60, 60)
	ToggleButton.Position = UDim2.new(0, 10, 0, 10)
	ToggleButton.BackgroundColor3 = Window.Color
	ToggleButton.BackgroundTransparency = 0.3
	ToggleButton.Image = Window.ToggleImage or "rbxassetid://3926305904"
...
function Assets:Dropdown(Parent,ScreenAsset,Window,Dropdown)
    ...
    ScrollingFrame.ScrollBarThickness = UserInputService.TouchEnabled and 8 or 4
    ...
    OptionContainerAsset.ZIndex = 1000
    ...
    local function ToggleDropdown()
        if not isDropdownOpen and ListLayout.AbsoluteContentSize.Y ~= 0 then
            ...
            local maxHeight = UserInputService.TouchEnabled and 200 or 150
            ...
            local backdrop = ScreenAsset:FindFirstChild("DropdownBackdrop")
            if backdrop then
                backdrop:Destroy()
            end
        end
    ...
    if UserInputService.TouchEnabled then
        local touchStartPos, touchStartTime, isSwiping = nil, nil, false
        ...
        DropdownAsset.InputBegan:Connect(function(input)
            if input.UserInputType == Enum.UserInputType.Touch then
                ...
                touchStartTime = tick()
                isSwiping = false
                DropdownAsset.BackgroundTransparency = 0.95
            end
        end)
        ...
        DropdownAsset.InputEnded:Connect(function(input)
            if input.UserInputType == Enum.UserInputType.Touch and touchStartPos and not isSwiping then
                ...
                if touchDuration < 0.4 then
                    ToggleDropdown()
                end
            end
            DropdownAsset.BackgroundTransparency = 1
            touchStartPos = nil
            isSwiping = false
        end)
    end
    
    -- Also handle Background specifically for better touch area
    DropdownAsset.Background.InputBegan:Connect(function(input)
        if input.UserInputType == Enum.UserInputType.Touch then
            task.wait(0.1)
            ToggleDropdown()
        end
    ...
    local function AddOption(Option,AddToList,Order)
        ...
        -- Handle option selection - works for both PC and mobile
        ...
        -- Mobile touch with proper handling
        if UserInputService.TouchEnabled then
            ...
            OptionAsset.InputBegan:Connect(function(input)
                if input.UserInputType == Enum.UserInputType.Touch then
                    ...
                    isTouching = true
                    -- Visual feedback
                    OptionAsset.BackgroundTransparency = 0.8
                end
            end)
            ...
            OptionAsset.InputChanged:Connect(function(input)
                if input.UserInputType == Enum.UserInputType.Touch and touchStartPos then
                    local delta = (input.Position - touchStartPos).Magnitude
                    if delta > 20 then
                        isTouching = false -- It's a swipe, not a tap
                    end
                end
            end)
            ...
            OptionAsset.InputEnded:Connect(function(input)
                if input.UserInputType == Enum.UserInputType.Touch then
                    OptionAsset.BackgroundTransparency = 1
...
function Bracket:Window(Window)
	...
	Window.ToggleImage = GetType(Window.ToggleImage,"rbxassetid://3926305904","string")
...
-- Helper function to add a default settings tab with common UI options
function Bracket:AddDefaultSettingsTab(Window, Tab)
	if not Tab then
		Tab = Window:Tab({Name = "Settings"})
	end
	
	-- UI Appearance Section
	local AppearanceSection = Tab:Section({Name = "UI Appearance", Side = "Left"})
	
	AppearanceSection:Label({Text = "Customize your UI theme and appearance"})
	
	local UIColorPicker = AppearanceSection:Colorpicker({
		Name = "UI Accent Color",
		Flag = "Settings/UIColor",
		Value = {0.05, 1, 1, 0, false},
		Callback = function(Value, Color)
			Window.Color = Color
		end
	})
	UIColorPicker:ToolTip("Change the accent color of the UI")
	
	local BackgroundTransparency = AppearanceSection:Slider({
		Name = "Background Transparency",
		Flag = "Settings/BGTransparency",
		Min = 0,
		Max = 100,
		Precise = 0,
		Unit = "%",
		...
		Callback = function(Value)
			Window.Background.ImageTransparency = Value / 100
		end
	})
	BackgroundTransparency:ToolTip("Adjust UI background transparency")
	...
	}):ToolTip("Enable blur effect behind UI")
	
	-- UI Size Presets
	AppearanceSection:Divider({Text = "Size Presets"})
	
	AppearanceSection:Button({
		Name = "Small (400x400)",
		Callback = function()
			Window.Size = UDim2.fromOffset(400, 400)
		end
	})
	...
	AppearanceSection:Button({
		Name = "Medium (496x496)",
		Callback = function()
			Window.Size = UDim2.fromOffset(496, 496)
		end
	})
	...
	AppearanceSection:Button({
		Name = "Large (600x600)",
		Callback = function()
			Window.Size = UDim2.fromOffset(600, 600)
		end
	})
	
	-- Mobile Settings Section
	if UserInputService.TouchEnabled then
		local MobileSection = Tab:Section({Name = "Mobile Options", Side = "Right"})
		
		MobileSection:Label({Text = "Optimizations for mobile devices"})
		
		MobileSection:Slider({
			Name = "Touch Sensitivity",
			Flag = "Settings/TouchSensitivity",
			Min = 50,
			Max = 150,
			Precise = 0,
			Unit = "%",
			Value = 100,
			Callback = function(Value)
				-- This is a placeholder - implement your sensitivity logic
			end
		}):ToolTip("Adjust touch input sensitivity")
		
		MobileSection:Toggle({
			Name = "Large Touch Targets",
			Flag = "Settings/LargeTouchTargets",
			Value = true,
			Callback = function(Value)
				-- Adjust minimum touch target sizes
			end
		}):ToolTip("Make buttons easier to tap on mobile")
		
		MobileSection:Divider({Text = "Performance"})
		
		MobileSection:Toggle({
			Name = "Reduce Animations",
			Flag = "Settings/ReduceAnimations",
			Value = false,
			Callback = function(Value)
				-- Implement animation reduction
			end
		}):ToolTip("Reduce animations for better performance")
	end
	
	-- General Settings Section
	local GeneralSection = Tab:Section({Name = "General", Side = "Right"})
	
	GeneralSection:Toggle({
		Name = "Show Watermark",
		Flag = "Settings/ShowWatermark",
		Value = false,
		Callback = function(Value)
			if Window.Watermark then
				Window.Watermark.Enabled = Value
			end
		end
	}):ToolTip("Toggle UI watermark visibility")
	
	GeneralSection:Toggle({
		Name = "Show Keybind List",
		Flag = "Settings/ShowKeybindList",
		Value = false,
		Callback = function(Value)
			if Window.KeybindList then
				Window.KeybindList.Enabled = Value
			end
		end
	}):ToolTip("Toggle keybind list visibility")
	
	GeneralSection:Divider({Text = "UI Controls"})
	...
	}):ToolTip("Center the UI on screen")
	...
	}):ToolTip("Reset UI to default position and size")
	
	GeneralSection:Keybind({
		Name = "Toggle UI",
		Flag = "Settings/ToggleUIKeybind",
		Value = "RightShift",
		Mouse = false,
		Callback = function(Key, IsDown, Toggle)
			if IsDown then
				Window.Enabled = not Window.Enabled
			end
		end
	}):ToolTip("Bind a key to toggle the UI")
